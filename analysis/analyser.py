...
from plotting.plotter import plot_layer_errors, plot_module_errors, plot_top_token_errors_by_module, plot_layer_magnitudes, plot_module_magnitudes, plot_top_token_magnitudes_by_module, plot_module_magnitudes_per_layer
...
class LLMAnalyser:
    ...
    @torch.no_grad()
    def run_activation_magnitude_analysis(self, num_samples: int = 16, plot: bool = False):
        """
        ...
        for module_suffix, token_mags_dict in token_mags_by_module.items():
            ...
            for token_id, mags in token_mags_dict.items():
                avg_token_mags.append((np.mean(mags), len(mags), self.tokenizer.decode(token_id)))
