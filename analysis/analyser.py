...
#     plot_layer_errors, plot_module_errors, plot_top_token_errors_by_module,
#     plot_layer_magnitudes, plot_module_magnitudes, plot_top_token_magnitudes_by_module,
#     plot_module_magnitudes_per_layer, plot_activation_kurtosis, plot_top_token_kurtosis
# )
...
#     @torch.no_grad()
#     def run_quantization_error_analysis(self, calib_dataset: str, bits: int, granularity: str, num_samples: int = 16, plot: bool = False):
#         self._register_hooks()
#         calib_data = get_calibration_data(calib_dataset, self.tokenizer, n_samples=num_samples)
#         if calib_data is None: self._remove_hooks(); return
...
#         print("\nCalculating activation quantization error...")
#         module_errors_list = defaultdict(list)
#         for i in tqdm(range(len(calib_data))):
#             self.model(calib_data[i]["input_ids"].to(self.device))
#             for name, activations in self.captured_activations.items():
#                 act_tensor = activations[0]
#                 quant_tensor = fake_quantize_activation(act_tensor, bits, granularity)
#                 module_errors_list[name].append(calculate_quantization_error(act_tensor, quant_tensor))
#             self.captured_activations.clear()
...
#     @torch.no_grad()
#     def run_per_token_error_analysis(self, calib_dataset: str, bits: int, granularity: str, plot: bool = False, num_samples: int = 16):
#         target_suffixes = ['self_attn.q_proj', 'self_attn.o_proj', 'mlp.gate_proj', 'mlp.down_proj']
#         self._register_hooks(module_names=target_suffixes)
#         calib_data = get_calibration_data(calib_dataset, self.tokenizer, n_samples=num_samples)
#         if calib_data is None: self._remove_hooks(); return
...
#             for name, activations in self.captured_activations.items():
#                 act_tensor = activations[0]
#                 suffix = next((s for s in target_suffixes if name.endswith(s)), None)
#                 if not suffix: continue
#                 quant_tensor = fake_quantize_activation(act_tensor, bits, granularity)
#                 per_token_mse = (act_tensor - quant_tensor).pow(2).mean(dim=-1).view(-1)
#                 for token_idx, error in enumerate(per_token_mse.tolist()):
#                     module_token_errors[suffix][input_ids_cpu[token_idx].item()].append(error)
#             self.captured_activations.clear()
...
#         module_top_tokens = {}
#         for suffix, errors_dict in module_token_errors.items():
#             avg_errors = []
#             for token_id, errors in errors_dict.items():
#                 avg_errors.append((np.mean(errors), np.std(errors), len(errors), token_id))
#             avg_errors.sort(key=lambda x: x[0], reverse=True)
#             all_avg_errors = [item[0] for item in avg_errors]
#             module_top_tokens[suffix] = {
#                 "top_tokens": [(e, s, c, self.tokenizer.decode(tid)) for e, s, c, tid in avg_errors[:10]],
#                 "median_mse": np.median(all_avg_errors) if all_avg_errors else 0.0
#             }
        
#         if plot:
#             plot_top_token_errors_by_module(module_top_tokens, self.model.config._name_or_path, bits, granularity)

#     @torch.no_grad()
#     def run_activation_magnitude_analysis(self, calib_dataset: str, num_samples: int = 16, plot: bool = False):
#         print("\nStarting unified activation magnitude analysis...")
#         self._register_hooks()
...
#         layer_mags, module_mags, token_mags_by_module, module_mags_per_layer = [defaultdict(list) for _ in range(4)]
#         token_target_suffixes = ['self_attn.q_proj', 'self_attn.o_proj', 'mlp.gate_proj', 'mlp.down_proj']

#         for i in tqdm(range(len(calib_data)), desc="Processing calibration samples"):
#             input_ids_gpu = calib_data[i]["input_ids"].to(self.device)
#             self.model(input_ids_gpu)
#             input_ids_cpu = calib_data[i]["input_ids"].view(-1)
...
#         torch.cuda.empty_cache()
#         self._remove_hooks()
#         self._process_and_plot_magnitude_results(
#             layer_mags, module_mags, module_mags_per_layer, token_mags_by_module, plot
#         )

#     def _process_and_plot_magnitude_results(self, layer_mags, module_mags, module_mags_per_layer, token_mags_by_module, plot):
#         model_name = self.model.config._name_or_path
        
#         avg_layer_mags = {layer: np.mean(mags) for layer, mags in layer_mags.items()}
#         print("\n--- Layer-wise Average Max Activation Magnitude ---")
#         for layer, avg_mag in sorted(avg_layer_mags.items()): print(f"  Layer {layer}: {avg_mag:.4f}")
#         if plot: plot_layer_magnitudes(avg_layer_mags, model_name)

#         avg_module_mags = {mtype: np.mean(mags) for mtype, mags in module_mags.items()}
#         print("\n--- Module-wise Average Max Activation Magnitude (All Layers)---")
#         for mtype, avg_mag in sorted(avg_module_mags.items()): print(f"  Module Type: {mtype:<20} | Avg. Max Magnitude: {avg_mag:.4f}")
#         if plot: plot_module_magnitudes(avg_module_mags, model_name)
        
#         avg_module_mags_per_layer = {mtype: {lidx: np.mean(mags) for lidx, mags in ldata.items()} for mtype, ldata in module_mags_per_layer.items()}
#         if plot: plot_module_magnitudes_per_layer(avg_module_mags_per_layer, model_name)

#         module_top_tokens = {}
#         print("\n--- Top Tokens by Average Max Magnitude ---")
#         for module_suffix, token_mags_dict in token_mags_by_module.items():
#             avg_token_mags = [(np.mean(mags), len(mags), self.tokenizer.decode(token_id)) for token_id, mags in token_mags_dict.items()]
#             avg_token_mags.sort(key=lambda x: x[0], reverse=True)
#             module_top_tokens[module_suffix] = avg_token_mags[:10]
#             print(f"\n  --- Module: {module_suffix} ---")
#             for avg_mag, count, token_text in module_top_tokens[module_suffix]: print(f"    Token: {repr(token_text):<15} (n={count}) | Avg. Max Magnitude: {avg_mag:.4f}")
#         if plot: plot_top_token_magnitudes_by_module(module_top_tokens, model_name)

#     @torch.no_grad()
#     def run_activation_kurtosis_analysis(self, calib_dataset: str, num_samples: int = 16, plot: bool = False):
#         print("\nStarting activation kurtosis analysis...")
#         self._register_hooks()
...
#         layer_kurtosis, module_kurtosis = defaultdict(list), defaultdict(list)
#         for i in tqdm(range(len(calib_data)), desc="Processing samples for kurtosis"):
#             self.model(calib_data[i]["input_ids"].to(self.device))
#             for name, activations in self.captured_activations.items():
#                 act_tensor = activations[0].to(torch.float32).view(-1).cpu().numpy()
#                 kurt_val = kurtosis(act_tensor, fisher=True)
#                 try:
#                     layer_index = int(name.split('.')[2])
#                     module_type = ".".join(name.split('.')[3:])
#                     layer_kurtosis[layer_index].append(kurt_val)
...
#         print("\n--- Layer-wise Average Activation Kurtosis ---")
#         for layer, avg_kurt in sorted(avg_layer_kurtosis.items()): print(f"  Layer {layer}: {avg_kurt:.4f}")
#         print("\n--- Module-wise Average Activation Kurtosis ---")
#         for mtype, avg_kurt in sorted(avg_module_kurtosis.items()): print(f"  Module Type: {mtype:<20} | Avg. Kurtosis: {avg_kurt:.4f}")
            
#         if plot:
#             plot_activation_kurtosis({"per_layer": avg_layer_kurtosis, "per_module": avg_module_kurtosis}, self.model.config._name_or_path)

#     @torch.no_grad()
#     def run_per_token_kurtosis_analysis(self, calib_dataset: str, num_samples: int = 16, plot: bool = False, layers_to_plot: list = [0, 15, 31]):
#         print("\nStarting per-token activation kurtosis analysis...")
#         target_module_suffixes = ['self_attn.q_proj', 'self_attn.o_proj', 'mlp.gate_proj', 'mlp.down_proj']
#         self._register_hooks()
...
#         module_token_kurtosis, layer_token_kurtosis = [defaultdict(lambda: defaultdict(list)) for _ in range(2)]
        
#         for i in tqdm(range(len(calib_data)), desc="Processing samples for per-token kurtosis"):
#             self.model(calib_data[i]["input_ids"].to(self.device))
#             input_ids_cpu = calib_data[i]["input_ids"].view(-1)
#             for name, activations in self.captured_activations.items():
#                 act_tensor = activations[0]
...
#                 for token_idx in range(act_tensor.shape[1]):
#                     token_act_vector = act_tensor[0, token_idx, :].to(torch.float32).cpu().numpy()
#                     kurt_val = kurtosis(token_act_vector, fisher=True)
#                     token_id = input_ids_cpu[token_idx].item()
                    
#                     if layer_index in layers_to_plot:
#                         layer_token_kurtosis[layer_index][token_id].append(kurt_val)
                    
#                     suffix = next((s for s in target_module_suffixes if name.endswith(s)), None)
#                     if suffix:
#                         module_token_kurtosis[suffix][token_id].append(kurt_val)
#             self.captured_activations.clear()
...
#         module_top_tokens = {
#             suffix: sorted([(np.mean(vals), len(vals), self.tokenizer.decode(tid)) for tid, vals in data.items()], key=lambda x: x[0], reverse=True)[:10]
#             for suffix, data in module_token_kurtosis.items()
#         }
#         if plot: plot_top_token_kurtosis(module_top_tokens, "Module", "top_token_kurtosis_by_module", model_name)

#         layer_top_tokens = {
#             layer_idx: sorted([(np.mean(vals), len(vals), self.tokenizer.decode(tid)) for tid, vals in data.items()], key=lambda x: x[0], reverse=True)[:10]
#             for layer_idx, data in layer_token_kurtosis.items()
#         }
#         if plot: plot_top_token_kurtosis(layer_top_tokens, "Layer", "top_token_kurtosis_by_layer", model_name)
